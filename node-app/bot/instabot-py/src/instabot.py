...
class InstaBot:
    ...
    def __init__(self,
        ...
        # Specify the log file to use, or leave blank for default
        self.log_file_name = log_file_name
        # Log file notification
        if self.log_file_name != '':
            if self.log_mod == 0:
                print("Logging:  %s %s" % (
                    "log_mod=0 set to console only, not using log file:",
                    self.log_file_name))
            elif self.log_mod == 1:
                print("Logging:  %s %s" % (
                    "log_mod=1 set to use log file:",
                     self.log_file_name))
            elif self.log_mod == 2:
                print("Logging:  %s %s" % (
                    "log_mod=2 set to disabled, not using log file:",
                    self.log_file_name))
    ...
    def write_log(self, log_text):
        ...
        if self.log_mod == 0:
            ...
            if self.log_file == 0:
                self.log_file = 1
                now_time = datetime.datetime.now()
                # Set log file name to use, otherwise use default name
                if self.log_file_name != '':
                    self.log_full_path = '%s%s' % (
